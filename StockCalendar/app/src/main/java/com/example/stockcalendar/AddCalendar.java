...
//检查是否已经添加了日历账户，如果没有添加先添加一个日历账户再查询
    ...
    //
//
    public static void addCalendarEvent(Context context,String title, String description, int[] beginTime){
        // 获取日历账户的id
        ...
        mCalendar.set(beginTime[0],beginTime[1]-1,beginTime[2]);//设置开始时间
        long start = mCalendar.getTime().getTime();
        mCalendar.setTimeInMillis(start + 1000*60*60*2);//设置终止时间
        long end = mCalendar.getTime().getTime();

        event.put(CalendarContract.Events.DTSTART, start);
        event.put(CalendarContract.Events.DTEND, end);
        event.put(CalendarContract.Events.HAS_ALARM, 1);//设置有闹钟提醒
        event.put(CalendarContract.Events.EVENT_TIMEZONE, "Taiwan/Taipei");  //这个是时区，必须有，
        //添加事件
